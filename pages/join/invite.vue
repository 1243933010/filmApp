<template>
	<view class="profix-page-container join-page has-tabbar">
		...
		<customHeader @listenLocale="listenLocale" :headerText="$t('app.name')" style="position: fixed; top: 0; width: 100%; z-index: 1; background: rgba(13, 13, 13, 1)" />
		<view class="banner1">
			<view style="position: absolute; top: 150rpx; left: 0rpx; display: flex; justify-content: center; align-items: center; font-size: 70rpx; color: white; width: 100%">
			...
			<view class="qucord">
				...
				<view class="qucord-text">
					...
					<view class="url">
						<view class="span"
							><text>{{ invitationObj.invitation_url }}</text>
						</view>
						<view class="copy">
							<image @click="copy(invitationObj.invitation_url)" src="../../static/img/copy_icon.png" mode="widthFix"></image>
						</view>
					</view>
				</view>
			</view>
						...
						<view class="invite-box-len-items">
							<scroll-view class="scroll-view_H" style="white-space: nowrap" scroll-x="true" @scroll="scroll" scroll-left="120">
								<view class="item" style="display: inline-block" v-for="(item, index) in inviteInfo.vip_user" :key="index">
									...
									<view class="hb-icon">
										<view class="icon">
											<view v-if="inviteInfo.vip_is_lock == 1" :style="{ backgroundImage: 'url(' + imgUrl + item.goods_img + ') ', backgroundSize: 'cover' }" class="hb-img">
												<view style="background-color: rgba(145, 145, 145, 0.5)">
													<image src="../../static/img/unuse.png" mode="widthFix" style="margin: 20rpx; width: 50rpx"></image>
												</view>
											</view>
											<view
												v-if="inviteInfo.vip_is_lock == 0 && item.is_draw == 1"
												:style="{ backgroundImage: 'url(' + imgUrl + item.goods_img + ') ', backgroundSize: 'cover' }"
												class="hb-img"
											>
												<view style="background-color: rgba(145, 145, 145, 0.5)">
													<image src="../../static/img/used.png" mode="widthFix" style="margin: 20rpx; width: 50rpx"></image>
												</view>
											</view>
											<view
												v-if="inviteInfo.vip_is_lock == 0 && item.is_standard == 0"
												:style="{ backgroundImage: 'url(' + imgUrl + item.goods_img + ') ', backgroundSize: 'cover' }"
												class="hb-img"
											>
												<view style="background-color: rgba(145, 145, 145, 0.5)">
											...
											<view
												v-if="inviteInfo.vip_is_lock == 0 && item.is_standard == 1 && item.is_draw == 0"
												:style="{ backgroundImage: 'url(' + imgUrl + item.goods_img + ') ', backgroundSize: 'cover' }"
												class="hb-img"
											>
											</view>
									...
									<view class="open-icon" @click="openDialog(item)" v-if="inviteInfo.vip_is_lock == 0 && item.is_draw == 0 && item.is_standard == 1">
